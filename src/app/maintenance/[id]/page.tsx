        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link href="/maintenance/workflow" className="text-gray-600 hover:text-gray-900">
                ← Back
              </Link>
              <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {workOrder.internalNotes && (
              <div className="bg-yellow-50 rounded-xl border border-yellow-200 p-6">
                ...
                <p className="text-gray-700 whitespace-pre-wrap">{workOrder.internalNotes}</p>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <div className="text-sm text-gray-900 font-medium">Work order created</div>
                ...
                {workOrder.updates.map((update) => (
                  <div key={update.id} className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <div className="text-sm text-gray-900">{update.note}</div>
                ...
                {workOrder.completedDate && (
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <div className="text-sm text-gray-900 font-medium">Work completed</div>
                      <div className="text-xs text-gray-500">{formatDate(workOrder.completedDate)}</div>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <dl className="space-y-3">
                <div>
                  ...
                  <dd className="text-sm text-gray-900 font-medium">{workOrder.category}</dd>
                </div>
                <div>
                  <dt className="text-xs text-gray-500">Reported By</dt>
                  <dd className="text-sm text-gray-900">{workOrder.reportedBy}</dd>
                  {workOrder.reportedEmail && (
                    <dd className="text-xs text-gray-500">{workOrder.reportedEmail}</dd>
                  )}
                </div>
                {workOrder.assignedTo && (
                  <div>
                    <dt className="text-xs text-gray-500">Assigned To</dt>
                    <dd className="text-sm text-gray-900">{workOrder.assignedTo}</dd>
                  </div>
                )}
                {workOrder.vendor && (
                  <div>
                    ...
                    {workOrder.vendor.company && (
                      <dd className="text-xs text-gray-500">{workOrder.vendor.company}</dd>
                    )}
                    {workOrder.vendor.phone && (
                      <dd className="text-xs text-gray-500">{workOrder.vendor.phone}</dd>
                    )}
                  </div>
                )}
                {workOrder.scheduledDate && (
                  <div>
                    ...
                    <dd className="text-sm text-gray-900">{formatDateOnly(workOrder.scheduledDate)}</dd>
                  </div>
                )}
              </dl>
            </div>
                ...
                <dl className="space-y-3">
                  {workOrder.estimatedCost && (
                    <div>
                      ...
                      <dd className="text-sm text-gray-900">${Number(workOrder.estimatedCost).toFixed(2)}</dd>
                    </div>
                  )}
                  {workOrder.actualCost && (
                    <div>
                      <dt className="text-xs text-gray-500">Actual Cost</dt>
                      <dd className="text-lg font-bold text-gray-900">${Number(workOrder.actualCost).toFixed(2)}</dd>
                    </div>
                  )}
                  {workOrder.paidBy && (
                    <div>
                      <dt className="text-xs text-gray-500">Paid By</dt>
                      <dd className="text-sm text-gray-900">{workOrder.paidBy}</dd>
                    </div>
                  )}
                  <div>
                  ...
                  {workOrder.invoiceNumber && (
                    <div>
                      ...
                      <dd className="text-sm text-gray-900">{workOrder.invoiceNumber}</dd>
                    </div>
                  )}
                  {workOrder.paidDate && (
                    <div>
                      <dt className="text-xs text-gray-500">Paid Date</dt>
                      <dd className="text-sm text-gray-900">{formatDateOnly(workOrder.paidDate)}</dd>
                    </div>
                  )}
                </dl>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <dl className="space-y-3">
                <div>
                  <dt className="text-xs text-gray-500">Property</dt>
                ...
                <div>
                  ...
                  <dd className="text-sm text-gray-900">Unit {workOrder.unit.unitNumber}</dd>
                ...
                {workOrder.lease && (
                  <div>
                    <dt className="text-xs text-gray-500">Current Tenant</dt>
                    <dd className="text-sm text-gray-900">
                    ...
                    {workOrder.lease.tenantPhone && (
                      <dd className="text-xs text-gray-500">{workOrder.lease.tenantPhone}</dd>
                    )}
                  </div>
                )}
              </dl>
